...
| 16 | 08-19→21 | The matrix prep died twice unwatched, and `afterany` released all 26 dependent jobs anyway; each bailed in under a second and the queue emptied | **48.5 hours of idle cluster.** Both failures were file-layout assumptions: a clinical cache keyed on the old bare `id`, and `days.npy` assumed present where a multi-day cohort carries `day_start.npy`. Fixed generally rather than case by case — see `docs/PITFALLS.md` §17 |
| 17 | 08-21 | A person who has to remember to check back is not a mechanism | **Three layers, and only the third is independent of a human**: `scripts/matrix_status.sh`, whose VERDICT defines STALLED as *queue empty and work unfinished*; a half-hourly watcher; and `scripts/pbs/B0_autolaunch.pbs` on `depend=afterok`, so a dead prep can no longer look like a live campaign |
| 18 | 08-22 | A read-only reviewer with no stake in the code was pointed at five areas of the repo. It found that the three-layer mechanism from row 17 had never once worked: `qstat` and `qsub` are in `/opt/pbs/bin`, cron's PATH is `/usr/bin:/bin`, and `$USER` is unset — so the monitor could not see the queue and the relaunch could not submit. Both failures wrote "OK" to the log | **A monitor that cannot observe must not conclude, and must never act.** Same error as row 16 in different clothes: "could not observe" read as "observed nothing". Every read is now rc-checked and anything unreadable returns `VERDICT: UNKNOWN`, which the watchdog logs and ignores. The relaunch no longer mints a throwaway anchor job (that qsub carried no `-P` and was rejected every time); the one-shot marker is written **after** a confirmed submission, not before |
| 19 | 08-22 | The same review measured the matrix that had been running for two hours: d1 cells held 107,927 windows and d7 cells 6,674, so a shared 100k steps × batch 64 is **62.7 epochs against 1016.2** — and controls, matched on one-day counts, were reused in all four cells, giving outlier `Loop/968` 5 windows against its control's 31 | **Killed the campaign at 11:47.** Row 15 already measured that epochs move arm AUC 0.680 → 0.940 at fixed T: a 16× epoch gap is *larger than the effect being measured*. Standardising `h` and `steps` equalises the optimiser's budget, not the training. `design.json` meanwhile reported the one-day matching figures for every cell, so reading it told you the opposite of the truth |
| 20 | 08-22 | You cannot hold windows *and* days constant while varying window length — a seven-day window is seven days | **Match on blocks, not totals.** Every cell now holds exactly the same 6,072 `(subject, day)` pairs: the intersection of the two seven-day cells' blocks, with the one-day cells matching on `days.npy` and the seven-day cells on `day_start.npy`. Equal counts ⇒ **1054 epochs in all four cells**; equal *per-subject* counts ⇒ control matching is identical everywhere (max gap 0.000, was 0.815). The residual — d7 sees 7× the raw signal — is what a longer window *is*, and is stated rather than hidden. `check_matrix_cells.py` now gates on exposure and epochs, not membership alone, and refuses the old cohorts |
| 21 | 08-22 | The reviewer also found `outliers/shape.py:120` indexes `cur` with medoid indices that address `W`, so B5 and B6 — 2 of the 13 consensus votes — score against three arbitrary windows | Checked before acting: **all 13 targets clear the 7-vote threshold with B5 and B6 discarded**, so no target is spurious and the running campaign stands. Three subjects sit at exactly 7 votes and may have been wrongly *excluded* — a coverage gap, not a wrong answer. Fix the bug, then decide whether to re-derive the consensus |
| 22 | 08-22 | The targets are the INTERSECTION — flagged by the consensus in all four seeds — but the controls were only screened against that same consensus, which is a far looser bar. `Loop/1041` was drawn as a "normal" carrying **19 method-flags out of 52** (13 methods × 4 seeds), against 28–47 for the real targets and 0 for the clean pool | **Screen both arms at the same strictness.** Controls now come from the complement of the UNION: the 376 of 506 subjects no method flagged in any seed. `Loop/1041` → `Loop/317`, matching still 0.000. Note this reverses `design.py`'s stated default, which kept borderline subjects in the pool to avoid a hand-picked "most normal" arm — that guards against a false *positive*, and this study has so far measured no leakage at all, so the false *negative* (a real outlier in the control arm makes both arms leak and shrinks the contrast) is the expensive direction. `matching["control_pool"]` records which rule was used, and `check_matrix_cells.py` now refuses to launch without the union audit — "not checked" must not read as "clean" |
| 23 | 08-22 | Re-derived the consensus with the B5/B6 medoid bug fixed, on a reconstructed full-record cohort (the battery had been scored on `matrix_d1_c1` at 107,927 windows, and that directory now holds the 6,072-window training cohort) | **The 13 targets did not move — none dropped, none added.** `sometimes` went 6 → 5 (`DCLP5/48` fell out) and the never-flagged pool widened 376 → 390. So two of thirteen votes had been scored against arbitrary windows for the whole study and the target set was robust to it. Worth knowing, and only knowable by running it |
| 24 | 08-22 | The three screens now sit at matched strictness: targets = consensus in **every** seed (13); background drops anyone the consensus flagged in **any** seed (5 removed, 475 left); controls come from the **390** subjects no method flagged in any seed | Final design gated: 501 subjects, 4 cells × 6,072 windows, **1054 epochs each**, control matching max gap **0.000**. `check_matrix_cells.py` refuses to launch without the union audit on disk — the campaign can no longer start on an unscreened control arm |
