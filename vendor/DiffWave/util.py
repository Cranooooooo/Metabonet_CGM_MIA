...
# Vendored from philsyn/DiffWave-unconditional for the m7mia framework.
# Made device-agnostic (original hardcoded `.cuda()`): tensor-creating helpers
# now derive their device from the input tensors / net parameters so the model
# runs on CPU (param counting, smoke tests) or GPU (training) unchanged.
...
def std_normal(size, device=None):
    ...
    `device` defaults to CUDA when available (preserving upstream behaviour) but
    may be any torch device string / object so the model runs on CPU as well.
    """
...
def calc_diffusion_hyperparams(T, beta_0, beta_T):
    ...
    a dictionary of diffusion hyperparameters including:
        ...
        These cpu tensors are moved to the relevant device by the caller.
    """
...
def sampling(net, size, diffusion_hyperparams):
    ...
    size (tuple):                   size of tensor to be generated,
                                    usually is (number of audios to generate, channels, length)
    diffusion_hyperparams (dict):   dictionary of diffusion hyperparameters returned by calc_diffusion_hyperparams
                                    note, the schedule tensors should be on the same device as net
...
def training_loss(net, loss_fn, X, diffusion_hyperparams):
    ...
    X (torch.tensor):               training data, shape=(batchsize, channels, length)
    diffusion_hyperparams (dict):   dictionary of diffusion hyperparameters returned by calc_diffusion_hyperparams
                                    note, the schedule tensors should be on the same device as X
    ...
    audio = X
    device = audio.device
    B, C, L = audio.shape  # B is batchsize, C=channels, L is length
