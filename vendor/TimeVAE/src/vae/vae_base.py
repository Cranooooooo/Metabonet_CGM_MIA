    ...
    def fit_on_data(self, train_data, max_epochs=1000, verbose=0):
        ...
        for epoch in range(max_epochs):
            ...
            for batch in train_loader:
                ...
                loss.backward()
                # Gradient clipping (added 2026-06-19): the wide dense layers at the
                # 4M/8M/16M size presets explode to NaN under Adam's default lr=1e-3
                # with no clipping (residual conns did not help). Clipping the global
                # grad norm contains the explosion; it is inactive for the stable 2M
                # preset, so the optimization recipe stays comparable across sizes.
