...
\subsection{Re-identification risk is not uniform across patients and
concentrates in those with atypical glucose profiles}\label{subsec:concentration}
...
The strongest published generator in our comparison, DiM-TS, exposes
\textbf{24 of 26} patients on the glucose-only composition against a floor of
\textbf{7}, an excess of 17 patients, and 15 against a floor of 8 once basal
insulin is added. No other learned generator approaches this. DiffWave, whose
fidelity is an order of magnitude worse, sits at an excess of 4 and 2;
Diffusion-TS at $-1$ and 4; FourierDiffusion at 2 and 3. Fidelity and exposure
are therefore not two ends of a single axis: the two best-fitting learned
generators in the table occupy opposite extremes of the exposure column.

TODO --- the concentration within a leaking release. State the outlier/control
split of the 24 exposed patients, the median per-patient AUC in each arm, and
the identity and score of the most exposed subject.

TODO --- one closing sentence carrying the clinical reading: the patients who
are exposed are those whose glycaemic profiles least resemble the cohort's,
which is precisely the population that data sharing exists to preserve.
...
\subsection{Basal insulin is the identifying channel in real recordings, yet
adding it does not increase leakage from synthetic release}\label{subsec:channel}

TODO --- Fig.~\ref{fig:channel}. Direction is mixed across arms; see
\texttt{PAPER\_PLAN.md} \S5.4 before writing a direction into this text.
