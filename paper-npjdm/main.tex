...
Section~\ref{subsec:mechanism} showed that removing absolute level takes
exposure from 24 to 13 of 26 on the glucose-only composition, and from 15 to 6
of 26 --- at or below the floor --- when basal insulin is present. That
transformation is available to any custodian at release time: publish the window
in a level-free form and re-offset it with a randomly drawn starting value.

TODO --- the clinical cost of this transformation has NOT been measured. Compute
the twenty metrics on level-stripped, re-offset releases and add them as a third
column of Table~\ref{tab:utility}. Metrics defined on shape (variability,
excursion amplitude, CONGA) should survive; metrics defined on the value scale
(mean glucose, time in range, time below 54) cannot, and the section must say
which of the twenty fall on each side rather than implying all are preserved.

TODO --- report these as per-window distributions with interquartile ranges and
a distribution distance, not as means with standard deviations. Two metrics can
share a mean and a standard deviation and still have different distributions,
and the clinical reading depends on the tails.
%%============================================================%%
